import React from "react";
import "./Card.css";
import TimeAgo from "../utils/Timefunction,js";
import FormatNumbers from "../utils/formatnumbers";
import { Link } from "react-router";

export default function Card({ videodata, smaller, medium, larger }) {
  const { title, thumbnailUrl, channelId, views, createdAt, _id } = videodata;
  const { channelImage, name } = channelId;
  return (
    <div
      className={`card-container ${smaller ? "smaller" : ""} ${
        medium ? "medium" : ""
      } ${larger ? "larger" : ""}`}
    >
      <div className="top">
        <Link
          to={`/watch/${_id}`}
          style={{ textDecoration: "none", color: "inherit" }}
        >
          <img src={thumbnailUrl} alt="thumbnail" />
        </Link>
      </div>
      <div className="bottom">
        {larger ? (
          <>
            <div>
              <div className="video-info">
                <div className="video-title">
                  <Link
                    to={`/watch/${_id}`}
                    style={{ textDecoration: "none", color: "inherit" }}
                  >
                    {title}
                  </Link>
                </div>
                <div className="info">
                  <div>{FormatNumbers(views)} views</div>
                  <div>{TimeAgo(createdAt)}</div>
                </div>
              </div>
              <div className="larger-card">
                <div className="channel-img">
                  <img src={channelImage} alt="" height={40} width={40} />
                </div>
                <div className="video-channel">
                  <Link to={`/channel/${channelId._id}`}>{name}</Link>
                </div>
              </div>
            </div>
          </>
        ) : (
          <>
            {!medium && (
              <div className="channel-img">
                <img src={channelImage} alt="" height={40} width={40} />
              </div>
            )}
            <div className="video-info">
              <div className="video-title">
                <Link
                  to={`/watch/${_id}`}
                  style={{ textDecoration: "none", color: "inherit" }}
                >
                  {title}
                </Link>
              </div>
              {!medium && (
                <div className="video-channel">
                  <Link to={`/channel/${channelId._id}`}>{name}</Link>
                </div>
              )}
              <div className="info">
                <div>{FormatNumbers(views)} views</div>
                <div>{TimeAgo(createdAt)}</div>
              </div>
            </div>
          </>
        )}
      </div>
    </div>
  );
}
